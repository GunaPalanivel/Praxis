...
**Reward:**
- Correct: `+0.20`
- Incorrect: `-0.10` (encourages attempts, penalizes wrong guesses)
...
**Reward:** `+0.15` if correct target, `-0.05` if wrong target.
...
**Reward:** `+0.25` if the deployment was the root cause, `-0.05` if not.
