...
- Rewards are generated by the centralized engine in `server/reward.py`.
- The implementation clamps per-step rewards to `[0.001, 0.999]`.
- Wrong or premature actions receive near-zero credit (`0.001`) after penalties.
- Duplicate actions are penalized (-50%).
- Medium and Hard tasks apply per-step cost pressure to discourage aimless exploration (`0.003` for medium, `0.006` and `0.005` for hard tasks).
...
### Scoring (optimal path = 0.63)

| Action                                          | Reward  |
| ----------------------------------------------- | ------- |
| `query_logs service=auth timerange=5m`          | `+0.08` |
| `check_config service=auth`                     | `+0.10` |
| `check_metrics service=auth metric=error_rate`  | `+0.05` |
| `check_metrics service=auth metric=connections` | `+0.08` |
| `check_deps service=auth`                       | `+0.05` |
| `diagnose root_cause=bad_config`                | `+0.20` |
| `rollback_deploy service=auth`                  | `+0.25` |
| Wrong diagnosis                                 | `0.001` |
| Wrong remediation                               | `0.001` |
...
- Cache hit rate drops, but cache degradation is downstream of the database bottleneck.
- **Notification service** is reporting false-positive errors on the "push" pipeline, unrelated to the core DB lockup.
...
| `notification` | degraded      | Red-herring push pipeline failure                      |

### Scoring (optimal path = 0.458)

| Action                                                     | Reward   |
| ---------------------------------------------------------- | -------- |
| `query_logs service=api timerange=10m`                     | `+0.024` |
| `check_deps service=api`                                   | `+0.024` |
| `check_metrics service=database metric=connections`        | `+0.074` |
| `query_logs service=database timerange=15m`                | `+0.044` |
| `diagnose root_cause=db_connection_pool_exhausted`         | `+0.134` |
| `kill_query service=database query_id=runaway_analytics`   | `+0.084` |
| `scale_resource service=database resource=connection_pool` | `+0.074` |
| Escalate after at least 3 unique investigations            | `+0.084` |
| Wrong diagnosis                                            | `0.001`  |
| Wrong remediation or premature escalation                  | `0.001`  |
| Unknown command                                            | `0.001`  |
...
| Action                                                          | Reward   |
| --------------------------------------------------------------- | -------- |
| `query_logs service=frontend timerange=10m`                     | `+0.045` |
| `query_logs service=api timerange=10m`                          | `+0.045` |
| `query_logs service=auth timerange=10m`                         | `+0.045` |
| `check_deps service=frontend`                                   | `+0.025` |
| `check_metrics service=dns-resolver metric=resolution_failures` | `+0.092` |
| `query_logs service=dns-resolver timerange=30m`                 | `+0.092` |
| `check_config service=dns-resolver`                             | `+0.042` |
| `diagnose root_cause=dns_misconfiguration`                      | `+0.187` |
| `restart_service service=dns-resolver`                          | `+0.137` |
| `escalate` after evidence                                       | `+0.137` |
| Premature diagnosis                                             | `0.001`  |
| Premature escalation                                            | `0.001`  |
...
- Blind remediation of `dns-resolver` receives no credit.
...
### Scoring (optimal path = 0.475)

| Action                                       | Reward   |
| -------------------------------------------- | -------- |
| `query_logs service=worker timerange=10m`    | `+0.035` |
| `check_metrics service=worker metric=memory` | `+0.085` |
| `check_config service=worker`                | `+0.035` |
| `diagnose root_cause=large_batch_size_oom`   | `+0.135` |
| `rollback_deploy service=worker`             | `+0.185` |
