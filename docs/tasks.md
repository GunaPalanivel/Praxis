...
Praxis currently exposes two implemented tasks and one planned task. Each task
is deterministic: the same command sequence produces the same observations and
rewards every run.
...
This task remains part of the design plan but is not registered in the current
server. It should not appear in `/tasks` or `/health` until the Phase 5
scenario file and tests are added.
