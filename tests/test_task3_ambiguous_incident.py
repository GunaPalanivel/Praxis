...
class TestOptimalPath:
    ...
    # Terminal restart includes efficiency bonus when the incident resolves.
    # Direct ``scenario.step`` calls keep step_count=0, so the bonus is
    # 0.1 * (1 - 1/MAX_STEPS) = 0.1 * 24/25 on a 25-step task.
    EXPECTED_REWARDS = [
        0.045,
        0.045,
        0.045,
    ...
    def test_optimal_path_total_score(self):
        ...
        assert total == pytest.approx(0.71 + 0.1 * (24 / 25), abs=1e-6)
...
class TestEvidenceRules:
    ...
    def test_escalate_with_evidence_ends_episode(self):
        ...
        assert outcome.reward == pytest.approx(0.137 + 0.1 * (24 / 25), abs=1e-6)
        assert outcome.done is True
        assert outcome.incident_resolved is True
